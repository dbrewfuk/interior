    ...
    callback: function(data) {
      ...
      while(n < x) {
        ...
        if (parseLinks) {
          ...
          if (printUser) {
            op += '<span class="twitter-username">' + strip(authors[n].innerHTML) +
                '</span>';
          }
          op += '<p class="tweet-content">' + strip(tweets[n].innerHTML) + '</p>';
          if (printTime) {
            if (permalinks) {
              op += '<span class="time-posted"><a href="' + permalinksURL[n] +
                 '">' + times[n].getAttribute('aria-label') + '</a></span>';
            } else {
              op += '<span class="time-posted">' +
